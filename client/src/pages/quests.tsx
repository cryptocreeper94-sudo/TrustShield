...
const TIERS = [
  { name: "Bronze", minXp: 0, color: "from-sky-700 to-sky-900", icon: Star },
  { name: "Silver", minXp: 1000, color: "from-gray-300 to-gray-500", icon: Star },
  { name: "Gold", minXp: 5000, color: "from-teal-400 to-sky-500", icon: Crown },
  { name: "Platinum", minXp: 15000, color: "from-cyan-300 to-blue-500", icon: Crown },
  { name: "Diamond", minXp: 50000, color: "from-sky-400 to-pink-500", icon: Sparkles },
];
...
export default function Quests() {
  ...
  const getDifficultyColor = (difficulty: string) => {
    switch (difficulty) {
      ...
      case "medium": return "text-sky-400 bg-sky-500/20 border-sky-500/30";
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 mb-4">
                  ...
                  <div className="bg-[rgba(12,18,36,0.65)] backdrop-blur-xl border border-white/[0.08] rounded-xl p-3 text-center shadow-[0_0_20px_rgba(251,191,36,0.1)]">
                    <div className="text-xl sm:text-2xl font-bold text-sky-400 tracking-tight">{userXp.toLocaleString()}</div>
                    <div className="text-[10px] text-muted-foreground uppercase">Total XP</div>
                  </div>
                  <div className="bg-[rgba(12,18,36,0.65)] backdrop-blur-xl border border-white/[0.08] rounded-xl p-3 text-center shadow-[0_0_20px_rgba(14,165,233,0.1)]">
                    <div className="text-xl sm:text-2xl font-bold text-sky-400 tracking-tight truncate">{currentTier.name}</div>
            ...
            <GlassCard glow>
              <div className="p-4 sm:p-6">
                <h3 className="text-base sm:text-lg font-bold mb-4 flex items-center gap-2">
                  <Trophy className="w-5 h-5 text-sky-400 shrink-0" /> Tier Benefits
                </h3>
                ...
                <div className="p-6">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-lg font-bold flex items-center gap-2">
                      <Trophy className="w-5 h-5 text-sky-400" /> XP Leaderboard
                    </h3>
                    ...
                    {[
                      ...
                      >
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${
                          entry.rank === 1 ? 'bg-sky-500 text-black' :
                          entry.rank === 2 ? 'bg-gray-300 text-black' :
                          entry.rank === 3 ? 'bg-sky-700 text-white' :
                        ...
                        <div className="text-right">
                          <div className="font-bold text-sky-400">{entry.xp.toLocaleString()}</div>
