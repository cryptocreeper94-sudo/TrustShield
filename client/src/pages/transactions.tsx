...
const TX_TYPE_CONFIG: Record<string, { icon: any; color: string; label: string }> = {
  ...
  stake: { icon: () => <span className="text-xs">🔒</span>, color: "text-sky-400", label: "Stake" },
  claim: { icon: () => <span className="text-xs">🎁</span>, color: "text-green-400", label: "Claim" },
  bridge: { icon: () => <span className="text-xs">🌉</span>, color: "text-sky-400", label: "Bridge" },
};
...
const STAMP_CATEGORY_CONFIG: Record<string, { label: string; color: string; icon: string }> = {
  ...
  "subscription-activated": { label: "Subscription", color: "text-sky-400", icon: "⭐" },
  "shells-purchase": { label: "Shells", color: "text-sky-400", icon: "🐚" },
  "domain-registration": { label: "Domain", color: "text-cyan-400", icon: "🌐" },
  "nft-mint": { label: "NFT Mint", color: "text-pink-400", icon: "🖼️" },
  "business-approved": { label: "Business", color: "text-cyan-400", icon: "🏢" },
                        ...
                        <div className="text-right">
                          ...
                          <div className="flex items-center justify-end gap-1">
                            {tx.status === 'confirmed' ? (
                              ...
                              <Badge className="bg-sky-500/20 text-sky-400 text-[9px] py-0 h-4">
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <Badge className={`text-[9px] py-0 h-4 ${stamp.status === "confirmed" ? "bg-green-500/20 text-green-400" : "bg-sky-500/20 text-sky-400"}`}>
                      ...
                      <div className="space-y-1.5 pt-2 border-t border-white/5">
                        ...
                        {parsedMeta.shells && (
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="text-sky-400 font-medium">{Number(parsedMeta.shells).toLocaleString()}</span>
                          </div>
                        )}
                        {parsedMeta.domain && (
                          <div className="flex items-center justify-between text-xs">
                        ...
                        {parsedMeta.plan && (
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="text-sky-400 font-medium">{parsedMeta.plan.replace(/_/g, ' ')}</span>
                          </div>
                        )}
                      </div>
                    </GlassCard>
